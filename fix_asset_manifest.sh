...
sed -i '/  "files": {/a     "static/media/fut_icon.png": "static/media/fut_icon.png",' 'asset-manifest.json'
...
#awk -v target="$target_line" -v lines="${lines_to_add[*]}" '1; $0 ~ target { for (i=1; i in lines_to_add; i++) print lines[i] }' asset-manifest.json > "$temp_file"
#
